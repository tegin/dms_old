...
class DmsSecurityMixin(models.AbstractModel):
    ...
    def check_access(self, operation, raise_exception=False):
        try:
            ...
            return access_right and access_rule and self.check_access_groups(operation) == None
        except AccessError:
            if raise_exception:
                raise
            return False
    ...
    def filter_access_groups(self, operation):
        ...
        if group_ids:
            ...
            self.env.cr.execute(sql_query, [self.env.user.id])
            ids_with_access += list(
                map(lambda val: val[0], self.env.cr.fetchall()))
